...
class BaseAPI(object):
    ...
    def _request(self, method, api, payload):
        if self.token:
            payload['token'] = self.token
            # kwargs.setdefault('params', {})['token'] = self.token

        try:
            logging.info(API_BASE_URL.format(api=api))
            logging.info(payload['token'])
    ...
    def get(self, api, params):
        return self._request(urlfetch.GET, api, params)

    def post(self, api, params):
        return self._request(urlfetch.POST, api, params)
...
class Users(BaseAPI):
    ...
    def list(self, presence=False):
        return self.post('users.list', params={'presence': int(presence)})
